...
$this->title = 'Імпорт лічильників води';
?>
    ...
    <div>
        <?php if ($isRun): ?>
            <div class="progress mt-3 mb-3">
                <?php if ($progress == 0): ?>
                    <div id="progress_bar" data-progress="<?php echo $progress; ?>" data-url="<?php echo Url::toRoute(['/admin/data/progress', 'id' => Tasks::IMPORT_COUNTERS_CSV]); ?>" class="progress-bar bg-info progress-bar-animated" role="progressbar" style="width:100%;font-weight:bold;">очікується обробка...</div>
                <?php else: ?>
                    <div id="progress_bar" data-progress="<?php echo $progress; ?>" data-url="<?php echo Url::toRoute(['/admin/data/progress', 'id' => Tasks::IMPORT_COUNTERS_CSV]); ?>" class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style="width:0;font-weight:bold;"></div>
                <?php endif; ?>
            </div>
        <?php else: ?>
        ...
        <div class="row">
            <div class="col">
                <div class="p-2 hint-block text-monospace mt-5">
                    <?php
                    $variants = CsvCounters::getVariants($client);
                    foreach (CsvCounters::$fields as $field => $label):
                        ?>
                        <p class="mb-2">- <?php echo $label; ?>
                            <?php
                            if (isset(CsvCounters::$notes[$field]) || isset($variants[$field])):
                                echo '(';
                            endif;
                            if (isset(CsvCounters::$notes[$field])):
                                echo CsvCounters::$notes[$field];
                            endif;
                            if (isset(CsvCounters::$notes[$field]) && isset($variants[$field])): echo ';';
                            endif;
